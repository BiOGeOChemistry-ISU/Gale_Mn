...
library(readxl)
ManganeseData_ForRCode <- read_excel("ManganeseData_ForRCode.xlsx", 
                        sheet = "Sheet1")
...
ManganeseData_ForRCode$log_Li_Mn=log10(ManganeseData_ForRCode$Li/ManganeseData_ForRCode$Mn_ppm)


###ANOVA Test####

## ANOVA based on TYPE: Carbonates, Freshwater, Marine
    #filtering to exclude NA and Gale Crater samples
clean_Data_Type<-filter(ManganeseData_ForRCode,Type=="Carbonates"|Type=="Freshwater"|Type=="Marine")

  ##ANOVA test and summary - To modify this code, change element.
model <- aov(log_Li_Mn ~ Type, data = clean_Data_Type)
summary(model)

  #to visualize plot
plot <- ggplot(clean_Data_Type, aes(x = Type, y = log_Li_Mn)) +
  geom_boxplot() +
  geom_jitter(width=0.1)+
  labs(title = "Box Plot of Value by Type", x = "Type", y = "log(Li/Mn)") + #change label here
  theme_minimal()
plot
...
## ANOVA based on FORMATION: Hydrogenetic, Diagenetic, Mixed, Freshwater, Carbonates
#filtering to exclude NA and Gale Crater samples
clean_Data_Formation<-filter(ManganeseData_ForRCode,Formation=="Hydrogenetic"|Formation=="Diagenetic"|Formation=="Mixed"|Formation=="Freshwater"|Formation=="Carbonates")

##ANOVA test and summary - To modify this code, change element.
model <- aov(log_Ba_Mn ~ Formation, data = clean_Data_Formation)
summary(model)

#to visualize plot
clean_Data_Formation$Formation <- factor(clean_Data_Formation$Formation , levels=c("Hydrogenetic", "Mixed", "Diagenetic", "Freshwater","Carbonates"))
plot <- ggplot(clean_Data_Formation, aes(x = Formation, y = log_Ba_Mn)) +
  geom_boxplot()+
  geom_jitter(width=0.1)+
  labs(title = "Box Plot of Value by Formation", x = "Type", y = "log(Ba/Mn)") +
  theme_minimal()
plot
...
Oxides_Carbonates<-filter(ManganeseData_ForRCode,Mn_Species=="Oxide"|Mn_Species=="Carbonate")
...
shapiro.test(Oxides$log_Zn_Mn) #Shapiro-Wilk normality test !!!Change element in this line!!!

Oxides<-Oxides$log_Zn_Mn #setting histogram  !!!Change element in this line!!!
hist(Oxides) #histogram
 
  #CARBONATES
Carbonates<-filter(Oxides_Carbonates,Mn_Species=="Carbonate") #filter
shapiro.test(Carbonates$log_Zn_Mn) #Shapiro-Wilk normality test !!!Change element in this line!!!

Carbonates<-Carbonates$log_Zn_Mn #setting histogram
hist(Carbonates) #histogram !!!Change element in this line!!!
  
  #FRESHWATER
Freshwater<-filter(Oxides_Carbonates,Type=="Freshwater") #filter
shapiro.test(Freshwater$log_Zn_Mn) #Shapiro-Wilk normality test !!!Change element in this line!!!

Freshwater<-Freshwater$log_Zn_Mn #setting histogram !!!Change element in this line!!!
hist(Freshwater) #histogram

  #MARINE
Marine<-filter(Oxides_Carbonates,Type=="Marine") #filter
shapiro.test(Marine$log_Zn_Mn) #Shapiro-Wilk normality test !!!Change element in this line!!!

Marine<-Marine$log_Zn_Mn #setting histogram !!!Change element in this line!!!
...
#boxplot
ggboxplot(
  Freshwater_Marine, x = "Formation", y = "log_Mn", #!!!Change element in this line!!!
  ylab = "log(Mn)", xlab = "Formation", add = "jitter" #!!!Change element in this line!!!
)
#add stat test
#stat test
stat.test <- Freshwater_Marine %>% 
  t_test(log_Mn ~ Formation) %>%
  add_significance()
stat.test <- stat.test %>% add_xy_position(x = "Formation")
bxp + 
