...
Full_Data <- read.csv("~/Documents/R/BiOGeOChemistry-ISU/Gale_Mn/Compiled_Lit_chemistry_withcategoricals_utf8.csv")
########################################### unit conversions #########################################
...
Full_Data <- Full_Data %>%
  mutate(across(all_of(names(molar_masses)), 
                ~ as.numeric(.x) / (molar_masses[cur_column()] * 1e6),
                .names = "{.col}_M"))
...
#CARBONATES
Carbonates<-filter(Full_Data,Mn.species=="carbonate") #filtering
...
Oxides <- Full_Data %>%
  filter(Mn.species == "oxide" & Formation %in% c("Freshwater", "Marine"))
...
plot_kruskal_boxplot <- function(data, y_var, y_label) {
  ...
  desired_order <- c("Carbonates", "Freshwater", "Hydrogenetic", "Diagenetic", "Mixed")
  ...
  # Check for sufficient data overall
  ...
  if (kruskal_result$p < 0.05) {
    # Check group sizes for Wilcoxon
    group_counts <- table(data$Pathway)
    
    if (any(group_counts < 2)) {
      warning("Not enough observations in one or more groups for Wilcoxon test")
      
      # Add annotation instead of post-hoc
      p <- p + annotate("text",
                        x = 3, y = max(data[[y_var]], na.rm = TRUE),
                        label = "Post-hoc skipped: insufficient data",
                        color = "red", size = 4)
    } else {
      # Safe to run Wilcoxon
      posthoc <- data %>%
...
write.csv(Oxides_Carbonates, "Oxides_Carbonates.csv", row.names = FALSE)
